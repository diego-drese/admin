...
use App\Library\MongoUtils;
use App\Models\RadioStation;

class ProcessRadioStation extends Command {
	/**
	 * The name and signature of the console command.
	 *
	 * @var string
	 */
	protected $signature = 'Aerd:ProcessRadioStation';
	...
	public function xml2array($array, $out = array()) {
		...
		foreach ($array as $index => $node){
			$index= str_replace('@','', mb_convert_encoding($index, 'ASCII', 'UTF-8' ));
		...
		while ($reader->read()) {
			...
			if ($nodeName == 'row') {
				...
				if(!empty($arrOutput['tipodocumento'])){
					...
					$estacaoRd = RadioStation::where('documento_historico.id_document',  $idDocument)->first();
					if(!$estacaoRd){
						RadioStation::where('id',  $arrOutput['id'])->push('documento_historico', $arrOutput);
					}
				}
			}
		}
	...
	public function processStacaoRd(){
		...
		while ($reader->read()) {
			...
			if ($nodeName == 'row') {
				...
				$estacaoRd = RadioStation::where('fistel', $arrOutput['fistel'])->first();
				if(!$estacaoRd){
					$arrOutput['client_id'] = null;
				}
				if(!$estacaoRd || $estacaoRd->checksum_stacao_rd!=$checksum){
					RadioStation::where('fistel',  $arrOutput['fistel'])
